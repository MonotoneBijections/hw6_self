...
def svd_compress(imArr, K=50):
    ...
    for ch in range(3):
        ...
        # --------------------
        # print(imArr[:,:,ch])
        u,s,v = np.linalg.svd(imArr[:,:,ch],full_matrices= False) 
        # get the 2d
        s_diag =  np.diag(s)  
        print(np.diag(s))
        # return the diagonal value in the matrix
        s[K:] = 0.0
        # mask out the other elements 
        imArr_compressed[:,:,ch] =np.dot(np.dot (u , s) , v)
